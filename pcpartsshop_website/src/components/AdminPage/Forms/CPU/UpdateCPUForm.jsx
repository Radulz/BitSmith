...
const UpdateCPUForm = ({ component, setComponent }) => {
  const {
    control,
    handleSubmit,
    register,
    ...
    defaultValues: {
      ...
      memoryFrequency: component.memoryFrequency,
      thermalDesignPower: component.thermalDesignPower,
      numberOfCores: component.numberOfCores,
    },
  ...
  const notify = (response) => {
    if (!response) {
      ...
      toast.success(
        `Component with ID: ${component.componentId} was updated successfully.`,
        {
          position: toast.POSITION.TOP_CENTER,
          autoClose: 5000,
        }
  ...
  const onSubmit = async (data) => {
    ...
    const response = await axios
      .put(process.env.REACT_APP_API_URL + `CPU/${component.componentId}`, {
        make: data.make,
        model: data.model,
